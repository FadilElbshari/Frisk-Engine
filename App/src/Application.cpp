...
namespace Frisk {
	...
	void Application::Run() {
		...
		shader->SetMat4("proj_matrix", glm::ortho(0.0f, static_cast<float>(1280), static_cast<float>(720), 0.0f, -2000.0f, 2000.0f));
		...
		while (!m_Window->ShouldClose()) {
			...
			for (U32 i{}; i < 128; i++) {
			    for (U32 j{}; j < 72; j++) {
			        renderer->Submitquad(VEC3(i*10, j*10, 0), VEC3(10, 10, 0), VEC3((i*j) % 256, (i+j) % 256, (int)abs((long double)(i-j)) % 256));
				}
			}
