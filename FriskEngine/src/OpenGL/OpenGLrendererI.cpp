...
namespace Frisk {
	...
	void OpenGLrendererI::Init() {
    	...
    	m_Data.VBOinstanced->SetLayout(VertexBufferLayout({ VertexBufferComponent("translation", VertexDataType::Float3, true), VertexBufferComponent("size", VertexDataType::Float3, true), VertexBufferComponent("color", VertexDataType::Float3, true) }));
	...
	void OpenGLrendererI::Endframe() {
        Assembleframe();
        m_Data.VAO->Bind();
        glDrawElementsInstanced(GL_TRIANGLES, m_Data.IBO->GetIndexCount(), GL_UNSIGNED_INT, nullptr, m_Data.InstanceCount);
	}
