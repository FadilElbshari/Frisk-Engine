...
namespace Frisk
{
    ...
    struct VertexBufferComponent
    {
        ...
        VertexBufferComponent(const STRING &a_Name, VertexDataType a_Type, bool a_Instanced = false,
                              bool a_Normalised = false)
            : name(a_Name), type(a_Type), size(GetSizeFromDataType(a_Type)), normalised(a_Normalised),
              instanced(a_Instanced), offset(0)
        {
        }
    ...
    struct VertexBufferLayout
    {
        ...
        const std::vector<VertexBufferComponent> &GetComponentList() const { return m_ComponentList; }
    ...
    class VertexBuffer
    {
      public:
        ...
        Create(const void *a_Verts, U32 a_Size); // for STATIC_DRAW: this will be mainly used for instanced drawing
